...
public:
    ...
    void beginCanOnly(const CanConfig& canConfig) noexcept
    {
        attachInterruptParam(/* pin_size_t       pin      */ canConfig.interrupt,
                             /* voidFuncPtrParam callback */ [](void*) {},
                             /* PinStatus        mode     */ LOW,
                             /* void*            param    */ nullptr);
    }
...
private:
    ...
    /// @brief Control Register Bit Mask Map
    ...
    {
        return static_cast<RegisterBitmask>(static_cast<uint8_t>(lhs) | static_cast<uint8_t>(rhs));
        // Why not use std::underlying_type_t<T> ? This is because RegisterBitmask cannot be anything other than uint8_t, since the register is a single byte. I don't want to include <type_traits> header.
    }
